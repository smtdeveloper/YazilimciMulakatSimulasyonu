...
Console.WriteLine("3.SORU OOP ilkeleri nelerdir açıkla, faydaları nedir ?");
...
// 2. Polimorfizm (Polymorphism)  : Bir nesnenin birçok şekle sahip olabilme yeteneği. Bu, nesnelerin farklı sınıflardan
// türetilebileceği anlamına gelir ve aynı yöntemlerin farklı nesne türleri için farklı şekillerde davranmasına izin verir.
// 3. Soyutlama (Abstraction : Karmaşık bir nesneyi basit ve anlaşılabilir parçalara ayırma yöntemidir.
// 4. Kapsülleme (Encapsulation) :  Nesnelerin iç yapısını gizleyerek, nesnelere yönelik işlemlerin sadece ilgili yöntemler
...
//
// 2. Open/Closed Principle (Açık/Kapalı Prensibi): Bir sınıfın veya fonksiyonun kaynak kodu değiştirilmeden genişletilebilir
// olması gerektiği prensibidir. Bu prensip, yeni özellikler eklemek için mevcut kodu değiştirmek yerine, mevcut kodu kullanarak
// yeni özellikler eklemenin mümkün olması gerektiğini ifade eder.
//
// 3. Liskov Substitution Principle (Liskov Yerine Geçme Prensibi): Bir sınıfın, türetilen sınıflar tarafından yerine
// konulabileceği prensibidir. Bu prensip, bir sınıfın, üst sınıfının yerine kullanılabilecek alt sınıfları desteklemesi
// gerektiğini ifade eder.
//
// 4. Interface Segregation Principle (Arayüz Ayrımı Prensibi): Kullanılmayan arayüzlerin sınıflara yüklenmemesi gerektiği
// prensibidir. Bu prensip, bir sınıfın, kullanmadığı özellikleri içermemesi gerektiğini ifade eder.
//
// 5. Dependency Inversion Principle (Bağımlılık Tersine Çevirme Prensibi): Yüksek seviyeli modüllerin düşük seviyeli
// modüllere bağlı olmaması gerektiği prensibidir. Bu prensip, bir yazılım bileşeninin diğer bileşenlerin detaylarına
// bağımlı olmaması gerektiğini ifade eder.
// sürdürebilirilik kod yazmak için  SOLID prensipleri, yazılım mühendisliği alanında kaliteli, esnek ve kolay bakım
// yapılabilir kod yazmak için temel ilkelerdir. 
...
//Bu genel çözüm önerileri, yazılımın yeniden kullanılabilirliğini arttırarak, kodun daha okunaklı, anlaşılır ve sürdürülebilir
...
//Değer tipleri, bellekte bir değer olarak saklanır ve doğrudan erişilebilirler. Bu türlerin örnekleri, tamsayılar (int),
...
//Referans tipleri, bellekte bir nesnenin adresi olarak saklanır ve dolaylı olarak erişilebilirler. Bu türlerin örnekleri,
//sınıflar (class),
...
//Bellekte saklanma şekilleri nedeniyle, değer tipleri genellikle daha hızlıdır ve daha az bellek kullanırlar.
//Ancak, referans tipleri,
